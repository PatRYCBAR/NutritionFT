...
$execute if score @s mp.nutr.toggle matches 1 run tellraw @s {"text":"$(confirm_on)","color":"#D6D6D6","italic":true}

#toggles setting off and displays confirmation message
$execute unless score @s mp.nutr.toggle matches 1 run tag @s add $(tag)
$execute unless score @s mp.nutr.toggle matches 1 run tellraw @s {"text":"$(confirm_off)","color":"#D6D6D6","italic":true}
